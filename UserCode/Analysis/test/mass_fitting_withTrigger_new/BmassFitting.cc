...
*/
   ...
   /* RooRealVar mu("mu", "mu", 5.36, 4, 6);
    ...
    RooRealVar x("Massdistribution", "M_{B^{0}_{s}} (GeV/c^{2})",4.0, 6.5);
    ...
    RooRealVar mean("mean", "Gaussian Mean", 5.36, 4, 6);
    RooRealVar sigma("sigma", "Gaussian Sigma", 0.1, 0.01, 10);
    RooGaussian gauss("gauss", "Gaussian PDF", x, mean, sigma);
    RooRealVar frac("frac", "Fraction", 0.1, 0.0, 1.0);
    RooAddPdf backgroundPdf("backgroundPdf", "Background Model", RooArgList(expo, gauss), RooArgList(frac));

    
    RooRealVar nbkg("N_B", "nbkg", 50000, 0, 500000);
    RooExtendPdf extendedBackgroundPdf("extendedBackgroundPdf", "Extended Background Model", polyBg, nbkg);
    RooRealVar nsig("N_S", "nsig", 10000, 0, 100000);
    RooExtendPdf extendedSignalPdf("extendedSignalPdf", "Extended Signal Model", gauss, nsig);
    ...
    double fraction = 1000000.0 / 1000000.0;  // Scaling factor for 100k events

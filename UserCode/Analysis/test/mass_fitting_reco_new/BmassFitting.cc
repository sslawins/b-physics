...
*/
   ...
   /* RooRealVar mu("mu", "mu", 5.36, 4, 6);
    ...
    RooRealVar mean("mean", "Gaussian Mean", 5.5, 4, 6);
    RooRealVar sigma("sigma", "Gaussian Sigma", 0.2, 0.005, 0.3);
    RooGaussian gauss("gauss", "Gaussian PDF", x, mean, sigma);

    RooRealVar mean2("mean2", "Gaussian Mean", 5.5, 4, 6);
    RooRealVar sigma2("sigma2", "Gaussian Sigma", 0.2, 0.01, 0.5);
    RooGaussian gauss2("gauss2", "Gaussian PDF", x, mean, sigma2);

    RooRealVar frac("frac", "Fraction", 0.5, 0.0, 1.0);
    RooAddPdf signalPdf("signalPdf", "Signal Model", RooArgList(gauss, gauss2), RooArgList(frac));

    
    RooRealVar nbkg("N_B", "nbkg", 5000, 0, 50000);
    RooExtendPdf extendedBackgroundPdf("extendedBackgroundPdf", "Extended Background Model", polyBg, nbkg);
    ...
    double L_eq_1M = 1e6/(0.5e-3*2*0.1)*1e-15; // from production

    double cms_lumi = 200.0;  // in fb⁻¹
    double fraction = 1597600.0 / 1000000.0;  // Scaling factor for 100k events

...
from ROOT import gStyle, TFile, TCanvas, gROOT, TLatex, TLegend
...
histograms = {
    ...
    'cBsMass': ('hBsMass', 'BsMass', 2, 1, False, False, None, None, None, None),
    'cBsMass_DirGenLenReco': ('hBsMass_DirGenLenReco', 'BsMass_DirGenLenReco', 2, 1, False, False, None, None, None, None),
    'cBsMass_DirRecoLenGen': ('hBsMass_DirRecoLenGen', 'BsMass_DirRecoLenGen', 1, 1, False, False, None, None, None, None),
...
hBsMass_DirGenLenReco = gROOT.FindObject('hBsMass_DirGenLenReco')
hBsMass_DirRecoLenGen = gROOT.FindObject('hBsMass_DirRecoLenGen')

cComp = TCanvas('cComp', 'cComp', 600, 600)
cComp.cd()

hBsComp = hBsMass.Clone('hBsComp')
hBsGenDirComp = hBsMass_DirGenLenReco.Clone('hBsGenDirComp')
hBsRecoDirComp = hBsMass_DirRecoLenGen.Clone('hBsRecoDirComp')

DrawHisto(hBsComp, 1, 1, 3)
hBsComp.SetFillStyle(3356)
DrawHisto(hBsGenDirComp, 2, 2, 3)
hBsGenDirComp.SetFillStyle(3384)
DrawHisto(hBsRecoDirComp, 0, 4, 3)
#hBsRecoDirComp.SetFillStyle(3305)

lComp = TLegend( 0.53 , 0.65 , 0.9 , 0.9 )
lComp.SetTextSize   (0.03)
lComp.AddEntry( hBsComp, 'Reconstructed (#gamma):' , '' )
lComp.AddEntry( hBsComp, 'Direction, magnitude' , 'l' )
lComp.AddEntry( hBsGenDirComp, 'Magnitude'  , 'l' )
lComp.AddEntry( hBsRecoDirComp,  'Direction'  , 'l' )

hBsComp.DrawCopy()
hBsGenDirComp.DrawCopy('same')
hBsRecoDirComp.DrawCopy('same')

lComp.Draw()
cComp.Draw()
cComp.Print("Plots/Comparison.png")
